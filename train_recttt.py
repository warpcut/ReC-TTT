...
from dataset import get_data_transforms, get_data_transforms_test
...
def train(args, device):
    ...
    optimizer = torch.optim.SGD(list(decoder.parameters()) + list(bn.parameters()), lr=1e-1, momentum=0.9, weight_decay=5e-4)
    optimizer2 = torch.optim.SGD(list(encoder.parameters()) + list(encoder2.parameters()) + \
                                list(classifier.parameters()) + list(classifier2.parameters()), lr=1e-1, momentum=0.9, weight_decay=5e-4)
