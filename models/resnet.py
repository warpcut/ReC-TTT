...
def load_my_state_dict(model, state_dict):
        ...
        for name, param in state_dict.items():
            if 'classifier' not in name:
                if 'module' in name:
                    own_state[name.replace('encoder.', '').replace('module.', '')].copy_(param)
                else:
                    own_state[name.replace('encoder.', '')].copy_(param)
        return own_state
...
def _resnet(
    ...
    if pretrained:
        if not custom:
            #state_dict = torch.load('.cache/torch/hub/checkpoints/resnet50-0676ba61.pth')
            state_dict =load_state_dict_from_url(model_urls[arch], progress=progress)
        else:
            print('Loading custom weights!')
            loaded_dict = torch.load('./saved_results/resnet_im32/weights/final.pth')['model_state_dict']
            state_dict = load_my_state_dict(model, loaded_dict)
...
def _resnet_c(
    ...
    model = ResNetC(block, layers, **kwargs)
    if pretrained:
        if not custom:
            ...
            print('Loading custom weights!')
            loaded_dict = torch.load('./saved_results/resnet50_im32_multi/weights/final.pth')['model_state_dict']
            state_dict = load_my_state_dict(model, loaded_dict)
        model.load_state_dict(state_dict)
    return model
...
def resnet50(pretrained: bool = False, progress: bool = True, custom: bool = False, **kwargs: Any):
    ...
    encoder = _resnet('resnet50', Bottleneck, [3, 4, 6, 3],
                      pretrained, progress, custom, **kwargs)
    if 'norm_layer' in kwargs:
        kwargs.pop('norm_layer')
    bn = BN_layer(Bottleneck, 3, **kwargs)
    return encoder, bn

def resnet50_c(pretrained: bool = False, progress: bool = True, custom: bool = False, **kwargs: Any):
    r"""ResNet-50 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`_.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    encoder = _resnet_c('resnet50', Bottleneck, [3, 4, 6, 3],
                      pretrained, progress, custom, **kwargs)
    if 'norm_layer' in kwargs:
        kwargs.pop('norm_layer')
    bn = BN_layer(Bottleneck, 3, **kwargs)
